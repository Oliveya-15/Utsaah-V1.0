  ...
  return (
    <ToastContext.Provider value={{ showToast }}>
      ...
      <div className="fixed bottom-5 right-5 z-[100] flex flex-col gap-2 max-w-sm w-[calc(100%-2.5rem)] sm:w-auto">
        {toasts.map((t) => (
          <div key={t.id} className="bg-white rounded-2xl shadow-card border-2 border-ink/5 px-4 py-3 flex items-center gap-2.5">
            {icons[t.type]}
            <p className="text-sm font-body font-semibold text-ink flex-1">{t.message}</p>
            <button onClick={() => removeToast(t.id)} className="text-ink/40 hover:text-ink shrink-0"><X size={16} /></button>
          </div>
        ))}
      </div>
